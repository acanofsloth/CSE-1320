...
void printAmerican(struct record info);
void britishRecords();
...
void printAmerican(struct record info)
{
    char american[10] = "American", check;
    ...
    for(int n = 0; n < MAX;n++)
    {
        ...
        for(int i = 0; i < LENGTH;i++)
        {
            if(american[i] != info.nation[n][i])
            {
                check = 'F';
                i += LENGTH;
            }
            if(american[i] == '\0' && info.nation[n][i] == '\0')
            {
                printf("%s\t\t%s\t\t%0.1f\n",info.firstName[n],info.lastName[n],info.score[n]);
                count++;
                total = total + info.score[n];
                i += LENGTH;
            }
        }
    }
    average = total/count;
    printf("Total= %0.1f\tCount= %d\tAverage= %0.1f\n\n",total,count,average);
}

void britishRecords()
{
    int n = 0,o, AMOUNT = 10, MAXLEN = 1000;
    FILE *pBritishFile;
    char fileContent[MAXLEN][AMOUNT],temp[LENGTH];
    char *token;
    pBritishFile = fopen("British.txt","r");
    if(!pBritishFile)
    {
        printf("\n\nFile Not Found! :(\n\n");
        return;
    ...
    while(fgets(fileContent[n],1000,pBritishFile))
    {
        n++;
    }
    fclose(pBritishFile);
